...
function render() {
 app.innerHTML=`${Navbar('restaurant')}<main class="restaurant-order-shell" id="restaurant-order-main"><section class="restaurant-order-hero compact-hero" style="--page-hero-image:url('${images.restaurant}')"><div class="page-hero-content"><p class="eyebrow">Harla Restaurant</p><h1>Restaurant Menu</h1><p>${labels[orderType]} · Fresh favourites, warm hospitality.</p></div></section><div class="order-container">${state.step==='menu'?menu():state.step==='details'?details():success()}</div></main>${Footer({ email: siteConfig.restaurantEmail })}`;
