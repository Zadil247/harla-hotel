...
export const priorityCountryCodes = ["ET", "US", "CA", "AU"];

export const countries = countryData.map(([code, name, dialCode, minDigits = 6, maxDigits = 14]) => ({
  code,
  name,
  dialCode,
  minDigits,
...
export function formatInternationalPhone(country, nationalNumber) {
  const digits = normalizeNationalNumber(nationalNumber);
  return country && digits ? `${country.dialCode}${digits}` : "";
}
