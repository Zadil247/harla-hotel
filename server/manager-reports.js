...
export const reportSources = [
  { key: 'rooms', table: 'room_bookings', label: 'Rooms', columns: 'id,booking_number,full_name,phone,email,room_type,check_in,check_out,number_of_rooms,status,payment_status,total_price_etb,total_price,created_at,updated_at', ref: 'booking_number', name: 'full_name' },
  { key: 'restaurant', table: 'restaurant_orders', label: 'Restaurant', columns: 'id,order_number,customer_name,phone,order_type,items,status,payment_method,payment_status,odoo_status,created_at,updated_at', ref: 'order_number', name: 'customer_name' },
  { key: 'events', table: 'event_hall_bookings', label: 'Events', columns: 'id,booking_reference,client_full_name,phone,email,hall_name,event_date,start_time,end_time,attendees,status,payment_status,quoted_amount,quoted_currency,created_at,updated_at', ref: 'booking_reference', name: 'client_full_name' },
  { key: 'tours', table: 'package_bookings', label: 'Tours', columns: 'id,package_name,full_name,phone,email,check_in,check_out,guests,message,status,created_at,updated_at', ref: 'id', name: 'full_name' },
  { key: 'tables', table: 'restaurant_requests', label: 'Table enquiries', columns: 'id,full_name,phone,email,reservation_date,reservation_time,guests,message,status,created_at,updated_at', ref: 'id', name: 'full_name' },
  { key: 'event_enquiries', table: 'event_requests', label: 'Event enquiries', columns: 'id,full_name,phone,email,event_type,event_date,guests,message,status,created_at,updated_at', ref: 'id', name: 'full_name' },
];
...
export async function buildWorkbook(data, summary = summarizeReport(data)) {
  ...
  for (const source of reportSources) {
    ...
    columns.push(['Service details', 'details', 60]);
    sheet(source.label, columns, (data.records[source.key] || []).map(row => ({
      reference: row[source.ref], customer: row[source.name], phone: row.phone, email: row.email,
